...
- 支持二阶段任务续写：先读取表格结构，再把读取结果交回模型生成真实写入计划
- 命令计划以 JSON 展示，包含：
  - `goal`
  - `summary`
  - `commands[].argv`
...
- 支持复制命令计划
- 支持 Dry Run
- 支持写入前弹窗确认，但默认信任本机 OfficeCLI 任务，避免每次写入都弹系统确认框
- 展示本地桥状态
- 展示执行日志、产物路径、HTML 预览
- 桌面助手自动检测和一键唤起
...
OfficeCLI 工具不是一次性猜表格结构。对“将材质列全部替换为塑料”这类任务，模型第一次通常只会生成读取计划，因为它还不知道真实 Sheet 名、表头在哪一行、材质列是第几列、数据到哪一行。

完整流程是：

1. 上传或填写本地 Excel 文件路径。
2. 输入任务，例如：`将材质列全部替换为塑料`。
3. 点击 `生成 OfficeCLI 计划`。
4. 如果生成的是 `只读计划`，点击 `演练计划` 或 `真实执行` 完成读取。
5. 执行结果区出现 `基于读取结果生成修改计划` 后点击它。
6. 模型会根据刚才读到的 stdout / logs / HTML 预览，生成真正的 `set` 或 `batch` 写入计划。
7. 确认命令计划无误后，关闭 `仅演练，不写文件`，再点击 `真实执行`。
8. 执行完成后查看输出文件和 HTML 预览。

如果读取结果仍不足以定位全部行，第二阶段会继续生成更充分的读取计划，例如把 `view text` 的 `--max-lines` 提高到 200，而不是凭空编造行号。再次执行读取后，再点一次 `基于读取结果生成修改计划` 即可继续。
